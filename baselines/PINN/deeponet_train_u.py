...
path = '/home/xinyang/storage/projects/PDE_structure/deeponet'#'./deeponet'
mkdir(path)
os.chdir('/home/xinyang/storage/projects/PDE_structure/deeponet')
...
def main():
    ...
    y_train = torch.load("training_data.pth", map_location=torch.device('cpu'))['u'].numpy()[:, 1:, 0, :-1, :-1].reshape(216, 100 * 1 * 256 * 256).astype(np.float32)
    y_test = torch.load('GroundTruth_test.pth', map_location=torch.device('cpu'))['u'].numpy()[:, 1:101, 0, :-1, :-1].reshape(100, 100 * 1 * 256 * 256).astype(np.float32)
    ...
    checker = dde.callbacks.ModelCheckpoint(path+"/model_u", save_better_only=False, period=100)
